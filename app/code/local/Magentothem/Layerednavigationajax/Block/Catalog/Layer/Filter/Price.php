...
class Magentothem_Layerednavigationajax_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price {
    ...
    public function setCurrentPrices() {
        ...
        if ((isset($_GET['q']) && !isset($_GET['first'])) || !isset($_GET['q'])) {
            if($direction == 'asc') {
                $searchMinPrice = $this->_productCollection->getFirstItem()->getPrice();
            } else {
                $searchMinPrice = $this->_productCollection->getLastItem()->getPrice();
            }
            if($rate) {
        ...
        if ((isset($_GET['q']) && !isset($_GET['last'])) || !isset($_GET['q'])) {
            if($direction == 'asc') {
                $searchMaxPrice = $this->_productCollection->getLastItem()->getPrice();
            } else {
                $searchMaxPrice = $this->_productCollection->getFirstItem()->getPrice();
            }
            if($rate) {
